...
//  neural
//
//  Created by Tomas Gareau on 2016-11-03.
...
std::vector<double> read_data( std::ifstream& ifstr, size_t num )
{
    std::vector<uint8_t> tmp( num );
...
std::vector<double> read_label( std::ifstream& ifstr )
{
...
int main()
{
    ...
    std::vector<Util::DataPair> training_data;
    ...
    {
        training_data.push_back( std::make_pair<std::vector<double>, std::vector<double>>( read_data( train_images, img_size ), read_label( train_labels ) ) );
    }
    ...
    std::vector<Util::DataPair> test_data;
    ...
    {
        test_data.push_back( std::make_pair<std::vector<double>, std::vector<double>>( read_data( test_images, test_img_size ), read_label( test_labels ) ) );
    }
